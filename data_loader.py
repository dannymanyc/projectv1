...
class BTCSequenceDataset(Dataset):
    def __init__(self, sequence_length=256, target_horizon=5):
        ...
        paths = sorted(Path(config.RAW_DATA_DIR).glob("*/*.parquet"))
        dfs = [pd.read_parquet(p) for p in paths]
        self.data = pd.concat(dfs, ignore_index=True).sort_values("timestamp").reset_index(drop=True)

        # Cache arrays
        self.close = self.data["close"].to_numpy(dtype=np.float32, copy=True)

        # Basic features (expand later if desired)
        self.X = self.data[["open", "high", "low", "close", "volume", "number_of_trades"]].to_numpy(dtype=np.float32)
